...
void insert_cache(cache_content *cache,int s,int t,unsigned int tag)
{
	for(int i=s;i<=t;i++)
	{
	...
	while(fscanf(fp, "%x", &x) != EOF)
    {
		...
		for(int i=0;i<way;i++)
		{
			if(cache[index][i].v && cache[index][i].tag==tag)
			{
				// hit
				hit_or_not=true;
				int index2=usable_cache(cache[index], way);
				insert_cache(cache[index], i+1, index2-1, tag);
				break;
			}
		}
		if(hit_or_not==0)
        {
        	...
        	if(index2==way)
        	{
        		insert_cache(cache[index], 1, way-1, tag);
        	}
        	else
        	{
        		cache[index][index2].v = true;
				cache[index][index2].tag = tag;
        	}
		}
	}
